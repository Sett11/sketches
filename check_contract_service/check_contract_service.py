...
class ContractChecker:
    ...
    def categorize_contract(self, contract_text: str) -> str:
        ...
        try:
            ...
            # Создаём общий промт для категоризации с ВСЕМИ категориями
            categories_list = ", ".join(categories)
            prompt = f"Определи категорию договора из следующего списка: {categories_list}. Текст договора: {contract_text}. Ответь ТОЛЬКО названием категории из списка."
            
            response = self.llm_client.generate_response(prompt)
            
            # Ищем точное совпадение с категориями из JSON
            response_clean = response.strip().lower()
            for category in categories:
                if category.lower() == response_clean or category.replace("_", " ").lower() == response_clean:
                    logger.info(f"Определена категория: {category}")
                    return category
            
            # Если категория не найдена в JSON - возвращаем None
            logger.warning(f"Category not found in JSON: {response}")
            return None
